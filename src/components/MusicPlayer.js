...
function MusicPlayer({ song, onPrev, onNext }) {
	...
	// problem - to many fetch calls on song click and on playing a song non stop fetch calls
	useEffect(() => {
			...
			try {
				const response = await fetch(
					`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(
						song.track.name
					)}&key=AIzaSyDb1i8QG2CVrsmyP-6aUaLo1_M4W4f8yzU&type=video&maxResults=1&videoCategoryId=10`
				);
